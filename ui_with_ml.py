...
class Connect4UI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.ai_types = ["Aléatoire", "Mini-max", "Modèle ML"]
        ...
        self.ml_model_label_var = tk.StringVar(value="")
        self.ml_load_button = ttk.Button(panel, text="Charger modèle ML", command=self.load_ml_model_dialog)
        self.ml_load_button.grid(row=10, column=0, sticky="ew", pady=(0, 6))
        self.ml_model_label = ttk.Label(panel, textvariable=self.ml_model_label_var, wraplength=230)
        self.ml_model_label.grid(row=11, column=0, sticky="w", pady=(0, 6))

        self.thinking_var = tk.StringVar(value="")
        ttk.Label(panel, textvariable=self.thinking_var).grid(row=12, column=0, sticky="w", pady=(0, 6))

        self._sync_ai_controls()

        # Boutons
        ttk.Button(panel, text="Nouvelle partie", command=self.new_game).grid(row=13, column=0, sticky="ew")
        ttk.Button(panel, text="Pause", command=self.pause_game).grid(row=14, column=0, sticky="ew", pady=(0, 6))
        ttk.Button(panel, text="Reprendre", command=self.resume_game).grid(row=15, column=0, sticky="ew")

        ttk.Button(panel, text="Retour (Undo)", command=self.undo_move).grid(row=16, column=0, sticky="ew", pady=(10, 6))
        ttk.Button(panel, text="Reretourner (Redo)", command=self.redo_move).grid(row=17, column=0, sticky="ew")

        ttk.Button(panel, text="Sauvegarder", command=self.save_game).grid(row=18, column=0, sticky="ew", pady=(10, 6))
        ttk.Button(panel, text="Charger", command=self.load_game).grid(row=19, column=0, sticky="ew")
        ...
        ttk.Button(panel, text="Paramétrage", command=self.open_settings).grid(row=21, column=0, sticky="ew", pady=(10, 0))
        if open_explorer_from_ui is not None:
            ttk.Button(panel, text="Explorer DB", command=lambda: open_explorer_from_ui(self)).grid(
                row=22, column=0, sticky="ew", pady=(10, 0)
            )

    def _sync_ai_controls(self):
        ai_type = self.ai_type_var.get()
        is_mm = ai_type == "Mini-max"
        is_ml = ai_type == "Modèle ML"
        self.ai_depth_spin.config(state=("normal" if is_mm else "disabled"))
        self.ml_load_button.config(state=("normal" if is_ml else "disabled"))
        if is_ml and self.ml_ai.is_loaded:
            self.ml_model_label_var.set(f"Modèle chargé: {Path(self.ml_ai.checkpoint_path).name}")
        elif is_ml:
            self.ml_model_label_var.set("Aucun modèle ML chargé")
        else:
            self.ml_model_label_var.set("")
    ...
    def ai_move(self):
        ...
        elif self.ai_type_var.get() == "Modèle ML":
            self.ai_move_ml()
        else:
            self.ai_move_random()

    def load_ml_model_dialog(self):
        ckpt = filedialog.askopenfilename(
            title="Choisir best_model.pt",
            filetypes=[("PyTorch model", "*.pt"), ("All files", "*.*")]
        )
        if not ckpt:
            return
        try:
            self.ml_ai.load(ckpt, device="cpu")
            self.ml_model_label_var.set(f"Modèle chargé: {Path(ckpt).name}")
            self._sync_ai_controls()
            messagebox.showinfo("Modèle ML", "Modèle chargé ✅\n" + ckpt)
        except Exception as e:
            messagebox.showerror("Modèle ML", "Impossible de charger le modèle.\n" + str(e))

    def ai_move_ml(self):
        if not self.ml_ai.is_loaded:
            self.status_var.set("Charge un modèle ML")
            messagebox.showwarning("Modèle ML", "Aucun modèle ML chargé. Clique sur 'Charger modèle ML'.")
            return
        try:
            col = self.ml_ai.choose_move(self.game.board, self.game.current_player)
        except Exception as e:
            messagebox.showerror("Modèle ML", "Erreur pendant l'inférence ML.\n" + str(e))
            return
        if col is None:
