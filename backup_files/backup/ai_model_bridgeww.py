...
class MLModelAI:
    """
    Drop-in replacement for ai_model_bridge.py.

    Behavior:
    - load the same best_model.pt checkpoint as before
    - compute the ML move
    - compute the minimax move at configurable depth
    - if both agree -> play that move
    - if they disagree -> play the minimax move

    Default depth is 7 so it matches your example, but you can change it in code
    or from the UI by calling set_minimax_depth(...).
    """
    ...
    def load(self, checkpoint_path: str, model_py_path: str | None = None, device: str = "cpu") -> None:
        ...
        self.last_debug = f"Loaded hybrid ML+Minimax(depth={self.minimax_depth}) from {ckpt_path.name}"
    ...
    def _choose_ml_move(self, board: List[List[object]], current_player: str) -> int:
    ...
    def _choose_minimax_move(self, board: List[List[object]], current_player: str) -> int:
        ...
        best_col = valid[0]
        best_score = -10**18
        for col in self.minimax_ai.ordered_valid_cols(mm_board, current_player, True):
            r = self.minimax_ai.next_open_row(mm_board, col)
            if r is None:
                continue
            mm_board[r][col] = current_player
            score = self.minimax_ai.minimax(mm_board, self.minimax_depth - 1, -10**18, 10**18, False, current_player)
            mm_board[r][col] = 0
            if score > best_score:
                best_score = score
                best_col = col
    ...
    def choose_move(self, board: List[List[object]], current_player: str) -> int:
        ...
        ml_move = self._choose_ml_move(board, current_player)
        mm_move = self._choose_minimax_move(board, current_player)

        if ml_move == mm_move:
            self.last_debug = f"Hybrid agree: ML={ml_move+1} MM={mm_move+1} -> play {ml_move+1}"
            return ml_move

        self.last_debug = (
            f"Hybrid disagree: ML={ml_move+1} MM={mm_move+1} depth={self.minimax_depth} -> play minimax {mm_move+1}"
        )
        return mm_move
