...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--skip-id-min", type=int, default=26052, help="Inclusive lower bound of id_partie range to skip")
    p.add_argument("--skip-id-max", type=int, default=49888, help="Inclusive upper bound of id_partie range to skip")
    return p.parse_args()
...
def fetch_games(args: argparse.Namespace) -> List[dict]:
    sql = """
        ...
        WHERE signature IS NOT NULL
          AND rows = %s
          AND cols = %s
          AND NOT (id_partie BETWEEN %s AND %s)
    ...
    try:
        with conn.cursor() as cur:
            params: List[object] = [args.rows, args.cols, args.skip_id_min, args.skip_id_max]
            if args.limit:
...
def save_bad_games_csv(path: str, records: List[dict]) -> None:
    fieldnames = [
        "split",
        "id_partie",
        "mode",
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for rec in records:
            writer.writerow(rec)
    print(f"[OK] Saved bad games CSV: {path} | rows={len(records)}")
...
def main() -> None:
    ...
    print(f"Skipped id range: [{args.skip_id_min}, {args.skip_id_max}]")
    ...
    print(f"Train kept/skipped games: {train[4]}/{train[5]}")
    print(f"Val kept/skipped games  : {val[4]}/{val[5]}")
    print(f"Test kept/skipped games : {test[4]}/{test[5]}")
