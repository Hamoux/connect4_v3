...
// ====== CLIENT ID (pour online / multi-onglets) ======
...
async function play(col) {
  ...
  // block IA click
  if (lastState?.type_partie === "IA" && lastState.current_player === lastState.ai_player) return;
...
function startPolling(){
  ...
  pollTimer = setInterval(async () => {
    ...
    const changed = JSON.stringify(data.board) !== JSON.stringify(lastState.board)
...
function render(state) {
  ...
  if (msg) {
    msg.hidden = false;
    if (state.game_over) {
      msg.innerHTML = `🏁 Victoire de <b>${escapeHtml(nameFor(state.current_player))}</b> !`;
    } else {
      msg.innerHTML = `Tour de <b>${escapeHtml(nameFor(state.current_player))}</b>`;
    }
  }
