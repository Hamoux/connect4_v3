...
async function postSetAiPrefs(depth) {
  const res = await fetch("/api/set_ai_prefs", {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ game_id: GAME_ID, client_id: CLIENT_ID, ai_depth: depth })
  });
  const data = await res.json();
  return { ok: res.ok, data };
}
...
async function postLocalAiMove(board, player, depth) {
  const res = await fetch("/api/local_ai_move", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ board, player, depth }) });
  const data = await res.json();
...
window.addEventListener("load", async () => {
  ...
  $("importSigFile")?.addEventListener("change", async (e) => {
    ...
    try {
      // L'import se base sur le nom du fichier, pas sur son contenu.
      // Exemple : 1234.txt => signature 1234.
      const sig = String(file.name || "")
        .replace(/\.[^.]+$/, "")
        .replace(/\s+/g, "")
        .trim();
      if (!sig || !/\d/.test(sig)) {
        if (statusEl) { statusEl.textContent = "Nom de fichier invalide : aucun chiffre trouvé."; statusEl.style.color = "var(--red, #f87171)"; }
        return;
      }

      const res = await postImportSignature(sig, "R");
      if (!res.ok) {
  ...
  $("diffSelect")?.addEventListener("change", async (e) => {
    ...
    if (hasActiveGame() && GAME_ID) {
      const res = await postSetAiPrefs(uiPrefs.difficulty);
