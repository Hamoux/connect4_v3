...
def has_immediate_winning_move(board, player):
...
def allows_opp_immediate_win(board, col, player):
    ...
    opp = 3 - player
    loses_next = has_immediate_winning_move(board, opp)
    undo_piece(board, row, col)
    return loses_next
...
def minimax(board, depth, alpha, beta, maximizing, player, tt, stats, rng, shuffle_ties):
    stats.nodes += 1
    ...
    key = (board_key(board), depth, maximizing, player)
    cached = tt.get(key)
    ...
    if maximizing:
        best_score = -math.inf
        best_cols = []
        ordered_cols = filter_safe_cols(board, order_moves(valid_cols, rng, shuffle_ties), player, stats)
        for col in ordered_cols:
            ...
            if is_winning_move(board, row, col, player):
                ...
                tt[key] = result
                return result
            score, _ = minimax(board, depth - 1, alpha, beta, False, player, tt, stats, rng, shuffle_ties)
            undo_piece(board, row, col)
        ...
        best_score = math.inf
        best_cols = []
        ordered_cols = order_moves(valid_cols, rng, shuffle_ties)
        for col in ordered_cols:
            ...
            if is_winning_move(board, row, col, opponent):
                ...
                tt[key] = result
                return result
            score, _ = minimax(board, depth - 1, alpha, beta, True, player, tt, stats, rng, shuffle_ties)
            undo_piece(board, row, col)
...
def pick_move(board, player, depth, rng, shuffle_ties=True, safe_filter=True):
    valid_cols = get_valid_cols(board)
    ...
    tt = {}
    stats = SearchStats()
    _, best_col = minimax(board, depth, -math.inf, math.inf, True, player, tt, stats, rng, shuffle_ties)

    ordered_valid = order_moves(valid_cols, rng, shuffle_ties)
    candidate_cols = ordered_valid
    if safe_filter:
        candidate_cols = filter_safe_cols(board, ordered_valid, player, stats)

    if best_col is None:
        best_col = candidate_cols[0]
    elif safe_filter and best_col not in candidate_cols:
        best_col = candidate_cols[0]
...
def play_game(depth=4, opening_random_plies=2, shuffle_ties=True, safe_filter=True, seed=None):
    ...
    while True:
        ...
        if forced is not None:
            ...
            if safe_filter:
                temp_stats = SearchStats()
                near_center = filter_safe_cols(board, near_center, player, temp_stats)
                total_safe_rejections += temp_stats.safe_rejections
                total_safe_fallbacks += temp_stats.safe_fallbacks
            col = rng.choice(valid)
        else:
            col, move_stats = pick_move(board, player, depth, rng, shuffle_ties=shuffle_ties, safe_filter=safe_filter)
        ...
        if row == -1:
            ...
            if safe_filter:
                temp_stats = SearchStats()
                fallback_valid = filter_safe_cols(board, valid, player, temp_stats)
                total_safe_rejections += temp_stats.safe_rejections
                total_safe_fallbacks += temp_stats.safe_fallbacks
            col = rng.choice(fallback_valid)
            row = drop_piece(board, col, player)
...
def worker_play_and_insert(depth, opening_random_plies, shuffle_ties, safe_filter, seed_base):
...
def main():
    ...
    parser.add_argument("--disable-safe-filter", action="store_true", help="Disable anti-suicide filtering")
    ...
    print(f"Anti-suicide filter   : {not args.disable_safe_filter}")
    print("=" * 80)
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            total_play_s += r["play_s"]
            total_dup_s += r["dup_s"]
            total_insert_s += r["insert_s"]
            total_end_to_end_s += r["total_s"]
            total_queries += r["queries"]
            total_nodes += r["nodes"]
            total_tt_hits += r["tt_hits"]
            total_cutoffs += r["cutoffs"]
            total_safe_rejections += r["safe_rejections"]
            total_safe_fallbacks += r["safe_fallbacks"]
            max_tt_size_seen = max(max_tt_size_seen, r["max_tt_size"])

            sig_text = format_sig(r["signature"], args.sig_preview)
            if r["result"] == "inserted":
                inserted += 1
                print(
                    f"✅ Game #{inserted:>5} | id={r['id_partie']} | {r['winner']:<15} | "
                    f"{r['moves']:>2} moves | sig: {sig_text} | play={r['play_s']:.3f}s | "
                    f"dup={r['dup_s']:.3f}s | insert={r['insert_s']:.3f}s | total={r['total_s']:.3f}s | "
                    f"q={r['queries']} | nodes={r['nodes']} | tt_hits={r['tt_hits']} | "
                    f"safe_rej={r['safe_rejections']}"
                )
            elif "duplicate" in r["result"]:
                duplicates += 1
                print(
                    f"⚠️  Duplicate skipped | sig: {sig_text} | play={r['play_s']:.3f}s | "
                    f"dup={r['dup_s']:.3f}s | total={r['total_s']:.3f}s"
                )
            else:
                errors += 1
    ...
    processed = inserted + duplicates + errors
    successful = inserted + duplicates

    avg_play = total_play_s / successful if successful else 0.0
    avg_dup = total_dup_s / successful if successful else 0.0
    avg_insert = total_insert_s / inserted if inserted else 0.0
    avg_total = total_end_to_end_s / successful if successful else 0.0
    avg_queries = total_queries / successful if successful else 0.0

    print("\n" + "=" * 80)
    print(f"Wall-clock run time             : {wall_clock_s:.2f}s")
    ...
    print(f"Average insert time / inserted  : {avg_insert:.4f}s")
    print(f"Average end-to-end / game       : {avg_total:.4f}s")
    print(f"Average SQL queries / game      : {avg_queries:.2f}")
    print(f"Total search nodes              : {total_nodes}")
    print(f"Total TT hits                   : {total_tt_hits}")
    print(f"Total alpha-beta cutoffs        : {total_cutoffs}")
    print(f"Total safe rejections           : {total_safe_rejections}")
    print(f"Total safe fallbacks            : {total_safe_fallbacks}")
    print(f"Max TT size seen                : {max_tt_size_seen}")
    print("-" * 80)
    ...
    print(f"Observed throughput by wall-clock      : {(inserted * 3600.0 / wall_clock_s) if wall_clock_s > 0 else 0.0:.1f}")
    print("=" * 80)
