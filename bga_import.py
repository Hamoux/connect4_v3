"""BGA import -> PostgreSQL.

Important:
- Connect Four on BGA can use the "inversion" opening rule (colors can swap).
  So we MUST rely on the *actual color per move* when available.
- The scraper emits moves with a "color" key ("R"/"J").
  This importer supports both formats:
    - new: {move_id, col, color, player_name?, player_id?}
    - old: {move_id, col, player_id} (best-effort mapping)
"""
...
def get_conn():
    ...
    # Never hardcode credentials. If password is None, psycopg2 will rely on local auth.
...
# ---------------- Import logic ----------------
def _canonical_signature_from_cols(cols_list, nb_cols: int) -> str:
    """Signature = min(cols, mirror(cols)). Mirror uses (nb_cols + 1 - col)."""
    s = "".join(str(int(c)) for c in cols_list)
    m = "".join(str(int(nb_cols + 1 - int(c))) for c in cols_list)
...
def import_bga_moves(moves, rows=9, cols=9, confiance=3):
    ...
    signature = _canonical_signature_from_cols(cols_seq, cols)
    ...
    # 5) Color resolution strategy
    # Preferred: moves already contain "color" (R/J) from gamereview logs.
    # Fallback (old format): map first seen player_id -> R, second -> J.
    pid_to_color = {}
    if any("color" in m for m in moves):
        pass
    else:
        pids = []
        for m in moves:
            pid_m = str(m.get("player_id"))
            if pid_m not in pids:
                pids.append(pid_m)
        if len(pids) < 2:
    ...
    for i, mv in enumerate(moves, start=1):
        ...
        if mv.get("color") in ("R", "J"):
            color = mv["color"]
        else:
            color = pid_to_color.get(str(mv.get("player_id")))

        if color not in ("R", "J"):
            raise ValueError(f"Impossible de déterminer la couleur pour le move #{i}: {mv}")
