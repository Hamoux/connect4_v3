...
class DBExplorer(tk.Toplevel):
    def __init__(self, master=None, config_path="config.json"):
        ...
        self.plateau_source_mode = tk.StringVar(value="auto")
    ...
    def _build_ui(self):
        ...
        ttk.Label(toggles, text="Source plateau:").pack(side="left", padx=(12, 4))
        source_combo = ttk.Combobox(
            toggles,
            textvariable=self.plateau_source_mode,
            values=("auto", "situation", "signature"),
            state="readonly",
            width=10,
        )
        source_combo.pack(side="left")
        source_combo.bind("<<ComboboxSelected>>", self._on_source_mode_change)
    ...
    def _build_virtual_situations_from_signature(self, sig: str):
        sig = sanitize_signature(sig)
        if not sig:
            return []

        board = empty_board(self.rows, self.cols)
        player = self.starting_player
        situations = []

        for numero, ch in enumerate(sig, start=1):
            col = int(ch) - 1
            if col < 0 or col >= self.cols:
                raise ValueError(f"Signature invalide: colonne hors bornes {ch}")
            ...
            if placed_row is None:
                raise ValueError(f"Signature invalide: colonne pleine {ch}")
    ...
    def _load_situations_for_partie(self, partie: dict):
        id_partie = partie["id_partie"]
        sig = partie.get("signature") or ""
        mode = (self.plateau_source_mode.get() or "auto").strip().lower()
        ...
        if mode == "situation":
            return db_situations, "situation (forced)"
        if mode == "signature":
            if sig:
                return self._build_virtual_situations_from_signature(sig), "signature (forced)"
            return [], "signature (forced, unavailable)"

        if db_situations:
            return db_situations, "situation"
        if sig:
            return self._build_virtual_situations_from_signature(sig), "signature (fallback)"
        return [], "none"
    ...
    def load_partie(self, id_partie: int):
        ...
        self.situations, source_label = self._load_situations_for_partie(partie)
    ...
    def _on_source_mode_change(self, _evt=None):
        if self.current_partie_id is not None:
            self.load_partie(self.current_partie_id)
